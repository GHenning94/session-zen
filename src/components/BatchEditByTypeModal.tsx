...
export function BatchEditByTypeModal({ open, onClose, onConfirm, selectedSessions }: BatchEditByTypeModalProps) {
  ...
  const hasChanges = useMemo(() => {
    if (sessionTypeInfo.type === 'individual') {
      return !!valor || !!metodo || !!status || !!anotacoes
    }
    return !!status || !!anotacoes
  }, [valor, metodo, status, anotacoes, sessionTypeInfo.type])
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-2 text-sm text-muted-foreground">
            <p><strong>Sessões individuais:</strong> Todos os campos podem ser editados.</p>
            <p><strong>Sessões recorrentes:</strong> Apenas status e anotações.</p>
            <p><strong>Sessões de pacote:</strong> Apenas status e anotações.</p>
          </div>
  ...
  // Package sessions - only status and notes
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-[450px]">
          ...
          <Alert className="bg-muted/50">
            <Info className="h-4 w-4" />
            <AlertDescription>
              Sessões de pacote: apenas status e anotações podem ser editados. O valor e método de pagamento são definidos no pacote.
            </AlertDescription>
          </Alert>
  ...
  // Individual sessions - full editing
  return (
    <Dialog open={open} onOpenChange={handleClose}>
