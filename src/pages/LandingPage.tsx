...
const LandingPage = () => {
  ...
  const plans = [
    { name: "Básico", price: { monthly: "R$ 0", annually: "R$ 0" }, period: { monthly: "/mês", annually: "/ano" }, annualTotal: 0, description: "Ideal para começar", features: ["Dashboard (versão limitada)", "Agenda completa", "Até 10 pacientes", "Até 10 sessões por paciente", "Prontuários (anamnese + anotações)", "Gerenciamento de pagamentos"], highlighted: false, cta: "Acessar", planId: "basico" },
    { name: "Profissional", price: { monthly: "R$ 29,90", annually: "R$ 24,90" }, period: { monthly: "/mês", annually: "/mês" }, annualTotal: 298.80, description: "Para profissionais em crescimento", features: ["Dashboard completo", "Até 50 pacientes", "Sessões ilimitadas", "Sessões recorrentes e pacotes", "Metas e relatórios padrão", "Prontuários completos", "Página pública de agendamento", "Programa de indicação", "Modo claro e escuro"], highlighted: true, cta: "Assinar Profissional", planId: "pro" },
    { name: "Premium", price: { monthly: "R$ 49,90", annually: "R$ 41,58" }, period: { monthly: "/mês", annually: "/mês" }, annualTotal: 498.96, description: "Máximo poder e recursos", features: ["Tudo do Profissional", "Pacientes ilimitados", "Programa de indicação completo", "Relatórios avançados (PDF, filtros)", "Integração Google Agenda", "Integração WhatsApp", "Personalização total (cores)", "Página pública personalizada", "Backup automático", "Suporte prioritário"], highlighted: false, cta: "Assinar Premium", planId: "premium" }
  ];
                ...
                {plans.map((plan) => (
                  <div key={plan.planId} className="fade-in-item">
                    <Card className={`flex flex-col h-full relative shadow-soft transition-all duration-300 ${plan.planId === 'pro' ? 'ring-2 ring-primary scale-105 shadow-primary hover:scale-110' : 'hover:-translate-y-2'}`}>
                      ...
                      {plan.planId === 'pro' && (
                        <Badge className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-primary text-[9px] px-1 py-0.5 flex items-center gap-0.5 whitespace-nowrap"><Star className="h-2 w-2" />Mais Popular</Badge>
                      )}
                      {/* Badge direita - Economize */}
                      {billingCycle === 'annually' && plan.planId !== 'basico' && (
                        <Badge className="absolute -top-3 right-2 bg-green-500 text-white text-[9px] px-1 py-0.5 whitespace-nowrap">Economize 2 meses</Badge>
                      )}
