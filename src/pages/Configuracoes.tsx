  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const { data: profileData, error: profileError } = await supabase
        .from('profiles')
        .select('id, user_id, nome, profissao, especialidade, bio, crp, telefone, telefone_codigo_pais, avatar_url, public_avatar_url, subscription_plan, billing_interval, created_at, updated_at, banco, agencia, conta, tipo_conta, cpf_cnpj, tipo_pessoa, nome_titular, chave_pix, bank_details_validated')
        .eq('user_id', user.id)
        .single();
                  ...
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#25D366]" aria-hidden>
                        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex gap-2">
                      <Select value={phoneCountryCode} onValueChange={handleProfilePhoneCountryChange}>
                        <SelectTrigger className="w-[80px] sm:w-[110px]">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
