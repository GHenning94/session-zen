...
import { formatPhone, formatCRP, formatCRM, validatePassword, formatCPFCNPJ, validateCPFCNPJ, formatPixKey, detectPixKeyType, PHONE_COUNTRIES, DEFAULT_PHONE_COUNTRY, formatInternationalPhone, getPhonePlaceholder, normalizePhoneDigits } from "@/utils/inputMasks"
...
const Configuracoes = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const profileSelectWithCountry = 'id, user_id, nome, profissao, especialidade, bio, crp, telefone, telefone_codigo_pais, avatar_url, public_avatar_url, subscription_plan, billing_interval, created_at, updated_at, banco, agencia, conta, tipo_conta, cpf_cnpj, tipo_pessoa, nome_titular, chave_pix, bank_details_validated';
      const profileSelectWithoutCountry = 'id, user_id, nome, profissao, especialidade, bio, crp, telefone, avatar_url, public_avatar_url, subscription_plan, billing_interval, created_at, updated_at, banco, agencia, conta, tipo_conta, cpf_cnpj, tipo_pessoa, nome_titular, chave_pix, bank_details_validated';
      ...
      const decryptedProfileData = await decryptSensitiveData('profiles', profileData as Record<string, any>);
      ...
      setPhoneCountryCode((decryptedProfileData as any)?.telefone_codigo_pais || DEFAULT_PHONE_COUNTRY);
  ...
  const handlePhoneChange = (value: string) => {
    handleSettingsChange('telefone', formatInternationalPhone(value, phoneCountryCode));
  };

  const handleProfilePhoneCountryChange = (newCode: string) => {
    if (settings.telefone) {
      const digits = normalizePhoneDigits(settings.telefone);
      handleSettingsChange('telefone', formatInternationalPhone(digits, newCode));
    }
    setPhoneCountryCode(newCode);
    handleSettingsChange('telefone_codigo_pais', newCode);
  };
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      const profileFields = ['nome', 'profissao', 'telefone', 'telefone_codigo_pais', 'crp', 'especialidade', 'bio', 'avatar_url', 'public_avatar_url', 'banco', 'agencia', 'conta', 'tipo_conta', 'cpf_cnpj'];
      ...
      // Salvar telefone apenas com dígitos (igual ao cadastro de paciente) e código do país
      if (profileData.telefone != null) {
        profileData.telefone = normalizePhoneDigits(profileData.telefone);
      }
      if (profileData.telefone_codigo_pais == null || profileData.telefone_codigo_pais === '') {
        profileData.telefone_codigo_pais = phoneCountryCode;
      }
      
      // Encrypt sensitive profile fields before saving
      const encryptedProfileData = await encryptSensitiveData('profiles', profileData);
      
      let { error: profileError } = await supabase.from('profiles').update(encryptedProfileData).eq('user_id', user.id);
      if (profileError && encryptedProfileData.telefone_codigo_pais != null && (profileError.message?.includes('telefone_codigo_pais') || /column.*does not exist/i.test(profileError.message))) {
        const { telefone_codigo_pais: _, ...dataWithoutCountry } = encryptedProfileData;
        const retry = await supabase.from('profiles').update(dataWithoutCountry).eq('user_id', user.id);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2">
                      <svg viewBox="0 0 24 24" className="h-5 w-5 shrink-0 text-[#25D366]" fill="currentColor" aria-hidden xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex gap-2">
                      <Select value={phoneCountryCode} onValueChange={handleProfilePhoneCountryChange}>
                      ...
                      <Input
                        type="tel"
                        value={settings.telefone ? formatInternationalPhone(settings.telefone, phoneCountryCode) : ''}
