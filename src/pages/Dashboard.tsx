...
const Dashboard = () => {
  ...
  const loadDashboardData = async (forceFresh = false, sections?: Array<keyof typeof cacheTimestamps>, isStale?: () => boolean) => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('sessions').select('id, data, horario, status, valor, client_id, clients(nome, avatar_url, medicamentos, eh_crianca_adolescente, tipo_atendimento)').eq('user_id', user?.id).eq('data', today).order('horario'),
  ...
  // IMPORTANTE: Usar mesma lógica de meses do calendário que os outros gráficos
  const loadCanalData = useCallback(async (period: '1' | '3' | '6' | '12') => {
    try {
      ...
      // Calcular range de datas baseado em MESES DO CALENDÁRIO (mesma lógica dos outros gráficos)
      ...
      // Se period=1, queremos apenas o mês atual
      // Se period=3, queremos os últimos 3 meses incluindo o atual
      ...
      paymentsData?.forEach((p: any) => {
        ...
        if (method === 'cartao_credito' || method === 'cartao_debito' || 
            ...
            method === 'cartão' || method === 'cartao de credito' || method === 'cartao de debito') {
          method = 'cartao'
        }
        // Normalizar transferência para transferencia
    ...
    <Layout>
      ...
      const getStatusLabel = (status: string) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:items-stretch">
          ...
          {/* Coluna da Direita - h-full para alinhar com a coluna esquerda */}
          <div className="space-y-6 flex flex-col h-full">
            ...
            {/* Clientes Recentes - flex-1 para preencher o espaço restante e alinhar com o card de canais */}
