-- CORREÇÃO: Ajustar função para não enviar '[]' como string no campo medicamentos
-- Modificar a função register_client_from_token para tratar medicamentos corretamente
...
BEGIN
    ...
    -- CORREÇÃO: Tratar medicamentos para evitar '[]' como string
    -- Converter jsonb array para text array, mas apenas se não estiver vazio
    IF p_client_data->>'medicamentos' IS NOT NULL AND p_client_data->>'medicamentos' != '' AND p_client_data->>'medicamentos' != '[]' THEN
        ...
        -- Se o array estiver vazio, setar como NULL
